...
const WatcherDefaultHandler = require('./handlers/watcher-default.js');

const RE_WATCH_PATH = /^.*\/jcr_root\/[^\/]*$/;

//** Watches for file system changes. */
class Watcher {
	constructor(sync, log) {
		this.sync = sync;
    this.log = log;
	...
	startChokidar(pathsToWatch, userFilter, ignored, callback) {
		...
		watcher.on('ready', function () {
			that.log.info(util.format('Found %s package folder(s).',
				pathsToWatch.length));

			// Just to print the message.
			that.helper.releaseLock(that.sync);

			// Detect all changes.
			watcher.on('all', function (eventName, localPath) {
				localPath = that.helper.cleanPath(localPath);
				that.log.debug('Change detected: ' + localPath);
    ...
    this.log.group();
		return this.helper.walkSync(workingDir, function (localPath, stats) {
