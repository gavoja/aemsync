...
async function init (breakStuff) {
  fs.removeSync(TEMP)
...
async function expect (message, expected) {
...
await expect('Excluded files and directories.', null)

// -----------------------------------------------------------------------------

add(`${COMPONENT}/file.txt`)
await expect('+ file.txt', {
  entries: [
    'META-INF/',
    'META-INF/vault/',
...
await expect('+ folder/sub-folder', {
  entries: [
    'META-INF/',
    'META-INF/vault/',
    'META-INF/vault/config.xml',
